...
export const STAGES: readonly StageDefinition[] = STAGE_TIERS.flatMap((tier) =>
  ...
  return {
    ...
    hpMultiplier: Number((1 + index * 0.05 + pressure * pressure * 1.35).toFixed(3)),
    damageMultiplier: Number((1 + index * 0.032 + pressure * 0.85).toFixed(3)),
    speedMultiplier: Number(Math.min(1.62, 1 + index * 0.004).toFixed(3)),
    levelHpGrowth: Number(Math.min(0.48, 0.21 + index * 0.0016).toFixed(3)),
    levelDamageGrowth: Number(Math.min(0.4, 0.17 + index * 0.0014).toFixed(3)),
