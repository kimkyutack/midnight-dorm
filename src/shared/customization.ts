...
export interface CosmeticDefinition {
  ...
  /** Cash-store product and therefore never a ghost-orb reward. */
  premium?: boolean;
  /** Package-only prestige reward. */
  prestige?: boolean;
}
...
const CHARACTERS = [
  ...
  { id: 'character-puppy', slot: 'character', label: '구름강아지 몽', description: '축 처진 귀와 동그란 코가 매력적', symbol: '멍', swatch: '#d8aa78', unlock: { kind: 'points', price: 600 } },
 ...
 */
const SKINS = [
  { id: 'skin-look-bunny-ward', slot: 'skin', characterId: 'character-bunny', traitMultiplier: 1.5, traitOverride: { label: '탐험가의 발걸음', description: '침대를 점유하기 전 이동속도가 1.5배가 됩니다.', unclaimedMoveSpeedMultiplier: 1.5 }, label: '탐험가 모모', description: '노란 안전모와 파란 후드의 완성형 이벤트 스킨', symbol: '토', swatch: '#e9c7bc', unlock: { kind: 'points', price: 500 } },
  ...
  { id: 'skin-look-puppy-surfer', slot: 'skin', characterId: 'character-puppy', premium: true, traitMultiplier: 2, traitOverride: { label: '파도 위 행운', description: '침대를 점유한 동안 매초 골드 5를 추가로 얻습니다.', goldPerSecond: 5 }, assetDirectory: 'skin-surfer-mong', label: '서퍼 몽', description: '하늘빛 고글과 보드를 타고 물결 위를 미끄러지는 완성형 스킨', symbol: '파', swatch: '#72d9f4', unlock: { kind: 'cash', price: 2_500 } },
  { id: 'skin-look-tiger-lifeguard', slot: 'skin', characterId: 'character-tiger', premium: true, traitMultiplier: 2, traitOverride: { label: '해변 구조 지휘', description: '포탑 사거리가 2칸 늘고 공격속도가 20% 증가합니다.', turretRangeBonus: 2, turretRateMultiplier: 1 / 1.2 }, assetDirectory: 'skin-lifeguard-raon', label: '해변 구조대 라온', description: '구명 튜브와 호루라기를 갖추고 물보라를 가르며 달리는 여름 한정 스킨', symbol: '구', swatch: '#ef5548', unlock: { kind: 'cash', price: 2_500 } },
  { id: 'skin-look-cat-neon-rider', slot: 'skin', characterId: 'character-cat', premium: true, traitMultiplier: 2, traitOverride: { label: '네온 오버클럭', description: '모든 포탑의 공격속도가 2배가 됩니다.', turretRateMultiplier: 0.5 }, assetDirectory: 'skin-neon-rider-lulu', label: '네온 라이더 루루', description: '네온 고글과 인라인 스케이트로 사이버 시티를 질주하는 프리미엄 스킨', symbol: '네', swatch: '#b347ff', unlock: { kind: 'cash', price: 2_500 } },
  { id: 'skin-look-hamster-cyber-driver', slot: 'skin', characterId: 'character-hamster', premium: true, traitMultiplier: 2, traitOverride: { label: 'Lv.5 양산 설계', description: '설치하는 모든 공격 포탑이 Lv.5로 시작합니다.', turretStartingLevel: 5 }, assetDirectory: 'skin-cyber-driver-kong', label: '사이버 드라이버 콩', description: '보랏빛 스포츠카와 무지개 휠로 네온 도로를 달리는 프리미엄 스킨', symbol: '카', swatch: '#803cff', unlock: { kind: 'cash', price: 2_500 } },
  { id: 'skin-look-crocodile-police-enforcer', slot: 'skin', characterId: 'character-crocodile', premium: true, traitMultiplier: 2, traitOverride: { label: '강력계 화력 지휘', description: '모든 포탑의 피해가 100% 증가하고 공격속도가 10% 증가합니다.', turretDamageMultiplier: 2, turretRateMultiplier: 1 / 1.1 }, assetDirectory: 'skin-police-enforcer-croco', label: '강력계 크로크', description: '압도적인 체격과 무전 장비로 현장을 장악하는 프리미엄 경찰 스킨', symbol: '경', swatch: '#315d8f', unlock: { kind: 'cash', price: 2_500 } },
  { id: 'skin-look-monkey-secret-agent', slot: 'skin', characterId: 'character-monkey', premium: true, traitMultiplier: 2, traitOverride: { label: '기밀 행운 조작', description: '랜덤상자 뽑기를 3회 더 사용하고 신화·전설 아이템 확률이 5%p 증가합니다.', extraDraws: 3, highRarityChanceBonus: 0.05 }, assetDirectory: 'skin-secret-agent-monkey', label: '시크릿 에이전트 몽키', description: '검은 수트와 쌍수 사격 자세로 임무를 수행하는 프리미엄 비밀요원 스킨', symbol: '첩', swatch: '#98754f', unlock: { kind: 'cash', price: 2_500 } },
  ...
  {
    id: 'skin-look-fox-moonlit-phantom',
    slot: 'skin',
    characterId: 'character-fox',
    prestige: true,
    traitMultiplier: 1,
    traitOverride: {
      label: '월령의 지배',
    ...
    label: '월령 환영 여우',
    description: '푸른 여우불과 월령 가면으로 병동의 밤을 지배하는 프레스티지 스킨',
    symbol: '월',
    swatch: '#43e6ff',
    unlock: { kind: 'reward' },
  },
  {
    id: 'skin-look-bunny-starlit-cloud',
    slot: 'skin',
    characterId: 'character-bunny',
    prestige: true,
    traitMultiplier: 1,
    traitOverride: {
      label: '천공의 축복',
    ...
    label: '성운 구름무희 모모',
    description: '별과 구름의 궤적을 두르고 천공 무대를 여는 프레스티지 스킨',
    symbol: '성',
    swatch: '#9ae9ff',
    unlock: { kind: 'reward' },
  },
  ...
  {
    id: 'skin-look-gorilla-abyssal-knight',
    slot: 'skin',
    characterId: 'character-gorilla',
    prestige: true,
    traitMultiplier: 1,
    traitOverride: {
      label: '흑염 군단장',
    ...
    label: '심연 기사단장 콩',
    description: '흑염 투구와 심연 방패로 전장을 장악하는 프레스티지 스킨',
    symbol: '심',
    swatch: '#e84d63',
    unlock: { kind: 'reward' },
  },
...
export const MOONLIT_PHANTOM_TILE_SKIN_ID = 'tile-moonlit-phantom';
...
export const MOONLIT_FOXFIRE_TURRET_SKIN_ID = 'turret-basic-moonlit-foxfire';
...
const TILE_SKINS = [
  ...
  {
    id: MOONLIT_PHANTOM_TILE_SKIN_ID,
    slot: 'tile',
    prestige: true,
    label: '월령 여우불 타일',
    description: '백덤블링 월령 여우가 쏜 불꽃이 방 전체를 태우며 타일을 전환합니다.',
    symbol: '령',
    swatch: '#31dfff',
    unlock: { kind: 'reward' },
    assetDirectory: 'skin-moonlit-phantom/moonfire-tile.webp',
  },
  {
    id: 'tile-starlit-cloud',
    slot: 'tile',
    prestige: true,
    label: '성운 구름무대 타일',
    description: '별자리와 구름결이 흐르는 천공 프레스티지 타일',
    symbol: '운',
    swatch: '#a5edff',
    unlock: { kind: 'reward' },
    assetDirectory: 'skin-starlit-cloud/starlit-cloud-tile.webp',
  },
  {
    id: 'tile-abyssal-knight',
    slot: 'tile',
    prestige: true,
    label: '심연 기사단 타일',
    description: '흑요석 균열과 흑염 문양이 살아나는 프레스티지 타일',
    symbol: '흑',
    swatch: '#e45466',
    unlock: { kind: 'reward' },
    assetDirectory: 'skin-abyssal-knight/abyssal-knight-tile.webp',
  },
] as const satisfies readonly CosmeticDefinition[];
...
const TURRET_SKINS = [
  ...
  {
    id: MOONLIT_FOXFIRE_TURRET_SKIN_ID,
    slot: 'turret',
    turretKind: 'basic-turret',
    prestige: true,
    label: '월령 천호포',
    description: '초승달 코어가 여우불 유도탄과 월식 충격파를 발사하는 프레스티지 포탑',
    symbol: '천',
    swatch: '#58e7ff',
    unlock: { kind: 'reward' },
    assetDirectory: 'skin-moonlit-foxfire',
  },
  {
    id: 'turret-basic-starlit-cloud',
    slot: 'turret',
    turretKind: 'basic-turret',
    prestige: true,
    label: '성운 성좌포',
    description: '별구름 코어가 프리즘 유성과 성운 파동을 발사하는 프레스티지 포탑',
    symbol: '성',
    swatch: '#8eeaff',
    unlock: { kind: 'reward' },
    assetDirectory: 'skin-starlit-cloud',
  },
  {
    id: 'turret-basic-abyssal-knight',
    slot: 'turret',
    turretKind: 'basic-turret',
    prestige: true,
    label: '심연 군단포',
    description: '흑염 기사단의 성문포가 지옥불 창과 심연 폭발을 발사합니다.',
    symbol: '심',
    swatch: '#ef5365',
    unlock: { kind: 'reward' },
    assetDirectory: 'skin-abyssal-knight',
  },
  { id: 'turret-basic-toy', slot: 'turret', turretKind: 'basic-turret', label: '수호포 · 장난감', description: '둥근 별 장식과 크림색 포신', symbol: '별', swatch: '#f1b86b', unlock: { kind: 'points', price: 400 } },
  { id: 'turret-basic-pumpkin', slot: 'turret', turretKind: 'basic-turret', label: '수호포 · 호박등', description: '주황빛 눈이 반짝이는 호박 포대', symbol: '호', swatch: '#e87942', unlock: { kind: 'points', price: 600 } },
  ...
  { id: 'turret-rapid-candy', slot: 'turret', turretKind: 'rapid-turret', label: '연사포 · 캔디팝', description: '분홍·민트 쌍열 포신 디자인', symbol: '팝', swatch: '#ed86b5', unlock: { kind: 'points', price: 500 } },
  { id: 'turret-rapid-dragon', slot: 'turret', turretKind: 'rapid-turret', label: '연사포 · 꼬마용', description: '작은 뿔과 입 모양 포구', symbol: '용', swatch: '#8ccf72', unlock: { kind: 'points', price: 800 } },
  ...
  { id: 'turret-frost-globe', slot: 'turret', turretKind: 'frost-turret', label: '서리포 · 스노우볼', description: '투명 구체 속 작은 눈보라', symbol: '설', swatch: '#c4f4ff', unlock: { kind: 'points', price: 600 } },
  { id: 'turret-frost-crystal', slot: 'turret', turretKind: 'frost-turret', label: '서리포 · 수정꽃', description: '육각 결정이 회전하는 희귀 외장', symbol: '정', swatch: '#7fc8ff', unlock: { kind: 'points', price: 900 } },
  ...
  { id: 'turret-arc-idol', slot: 'turret', turretKind: 'arc-turret', label: '천둥포 · 구름신상', description: '구름 고리가 번개를 모은다', symbol: '운', swatch: '#b69cf2', unlock: { kind: 'points', price: 950 } },
  { id: 'turret-arc-crown', slot: 'turret', turretKind: 'arc-turret', label: '천둥포 · 왕실폭뢰', description: '왕관 코어가 빛나는 최상급 외장', symbol: '왕', swatch: '#f0bd63', unlock: { kind: 'points', price: 1_200 } },
] as const satisfies readonly CosmeticDefinition[];
...
/** Prestige rewards are collection-only and never belong in the point storefront. */
export const cosmeticVisibleInPointShop = (item: CosmeticDefinition): boolean =>
  item.prestige !== true;
...
export function cosmeticAvailable(item: CosmeticDefinition, rank: RankId, owned: readonly string[]): boolean {
  ...
  if (item.unlock.kind === 'points' || item.unlock.kind === 'cash') return owned.includes(item.id);
  if (item.unlock.kind === 'reward') return owned.includes(item.id);
...
export function turretSkinAssetUrl(
  ...
  const safeLevel = Math.max(1, Math.min(item.prestige ? 17 : 15, Math.floor(level)));
