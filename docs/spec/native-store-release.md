...
| `com.midnightdorm.cash.10800` | 캐시 10,800개 | ₩120,000 |

클라이언트는 가격이나 지급량을 신뢰하지 않는다. 구매 토큰은 Worker에 한 번만 제출하고, 검증 완료 트랜잭션만 서버 원장에서 캐시를 지급한다. 각 SKU는 계정당 최초 1회에 한해 20% 캐시를 추가 지급한다. 예를 들어 10,800 캐시 팩의 첫 구매 지급량은 12,960 캐시이며, 이후에는 10,800 캐시다. 귀신구슬 소환처럼 캐시를 사용하는 기능은 서버에서 잔액 확인과 차감을 보상 지급과 같은 트랜잭션으로 처리한다.

기존 `com.midnightdorm.cash.10400`은 가격을 덮어쓰지 말고 판매 중지 또는 비공개 처리한다. 두 스토어 콘솔에는 새 소모성 SKU `com.midnightdorm.cash.10800`을 별도로 생성해 클라이언트·Worker의 상품 ID와 일치시킨다.
