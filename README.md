...
1. 최초 실행 시 복도 추격 티저를 본 뒤 계정을 만들거나 로그인합니다. 싱글/멀티 등급, XP와 스테이지 진행도는 D1에 저장됩니다.
2. 게임 홈에서 로그인 정보와 최고 표시 등급을 확인하고 `게임 시작`을 누릅니다.
3. 싱글/멀티 모드와 해금된 스테이지를 고르고 새 방을 만들면 8자리 초대 코드가 발급됩니다.
4. 다른 기기에서 같은 URL을 열고 초대 코드로 참가합니다.
5. 참가자가 준비하면 방장이 30초 준비 시간을 시작합니다. 골드는 즉시 생산되고 귀신은 30초 뒤 움직입니다.
6. 조이스틱으로 빈 방의 침대 가까이 이동해 표시되는 `잠자기`를 누릅니다.
7. 방의 `+` 타일을 눌러 골드·전력·보급 탭에서 수호 포탑, 서리 스프레이, 발전기와 방어 설비를 설치합니다. 설치 비용은 설비별로 골드 또는 전력 한 종류를 사용하며, 침대·문·발전기의 고레벨 강화에는 골드 비용의 10% 전력이 추가됩니다.
8. 모든 생존자가 재대결에 동의하면 같은 팀으로 다시 시작합니다.
...
- `src/shared/progression.ts`: 6개 등급, 185개 스테이지, 난이도 곡선과 싱글 등급 혜택
...
전투 수치와 계산식은 [`docs/spec/balance.md`](docs/spec/balance.md), 화면 스타일과 모바일 UX는 [`docs/spec/visual-design.md`](docs/spec/visual-design.md)를 기준으로 관리합니다.

- 싱글 등급과 멀티플레이 등급은 `하수 → 중수 → 고수 → 초고수 → 베테랑 → 레전드`로 별도 계산합니다. 게임 안에는 둘 중 높은 등급을 표시합니다.
...
- 싱글 등급은 시작 자원과 이동속도, 수호 포탑 최대 레벨을 높입니다. 문은 등급과 무관하게 10레벨까지 올릴 수 있습니다.
